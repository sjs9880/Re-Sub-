@ECHO OFF
TITLE Re-Sub! v2.b3 :: 자막 이름 매칭 스크립트

ECHO (C) 2019. _SN_(sjs9880) all rights reserved.

::영상 목록 추출
FOR /f "tokens=*" %%A IN ('DIR *.3gp *.asf *.avi *.dpl *.dsf *.flv *.mkv *.mov *.mp4 *.mpe *.mpeg *.mpg *.nsr *.ogm *.rmvb *.tp *.ts *.vob *.wm *.wmv /A:-D /O:N /B') DO ECHO %%A>>Vid
